...
margin_log_receiver(struct margin_recv *recv)
{
  ...
  margin_log("Margining dwell time: %d ms\n\n", recv->dwell_time);
  ...
  if (arg.verbosity > 0)
    {
      ...
      u64 lane_eta_s = (arg.steps_lane_total - arg.steps_lane_done) * arg.recv->dwell_time / 1000;
      u64 total_eta_s = *arg.steps_utility * arg.recv->dwell_time / 1000 + lane_eta_s;
